...
nums = map (readFunc . (splitRegex (mkRegex " "))) my_lines
       where
         readFunc x = (map read x :: [Integer])
         my_lines = splitRegex (mkRegex "\n") num_string

-- how we calculate the product of the 4 elements
my_prod = (concatMap lineFunc)
       where
         lineFunc = ((map product ). (map (take 4)) . tails)
